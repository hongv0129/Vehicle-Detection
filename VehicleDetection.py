...
def get_hog_features(img, orient, pix_per_cell, cell_per_block, vis=False, feature_vec=True):
    ...
    if vis == True:
        hog_features, hog_image = hog(img, orientations=orient, 
                                  ...
                                  transform_sqrt=boTransformSqrt, 
...
def bin_spatial(img, size=(32, 32)):
    ...
    #[1] Use cv2.resize().ravel() to create the feature vector
...
def extract_features(img_files, color_space='YCrCb', spatial_size=(32, 32), 
    ...
    for file in img_files:
        ...
        if hist_feat == True:
            ...
            color_hist_features = color_hist(feature_image, nbins=hist_bins, bins_range=hist_range)
...
def extract_single_image_features(img, color_space='RGB', spatial_size=(32, 32),
    ...
    if hog_vis == False:
        ...
        return np.concatenate(img_features), hog_image, feature_image[:,:,hog_channel]

#==============================================================================
# Functions for sliding window searching
#==============================================================================
...
#------------------------------------------------------------------------------
def draw_labeled_bboxes(img, labels, color=(0,0,255), thick=3):
    imgcopy = np.copy(img)
...
#------------------------------------------------------------------------------
def add_weight_to_heatmap(heatmap, bbox_list):
...
#------------------------------------------------------------------------------
def display_images_var1(fig, rows, cols, images, titles, cmap='hot'):
...
def find_cars(img, conv_color, ystart, ystop, scale, svc, X_scaler, orient, pix_per_cell, cell_per_block, spatial_size, hist_bins):
    ...
    for xb in range(nxsteps):
        for yb in range(nysteps):
            ...
            # Scale: 128, 160, searching area along x_axis: 150 to 400 and 800 to 1280
            # Scale: 112,      searching area in x_axis: 0 to 480 and 720 to 1280
            # Scale:  64, 96,  searching area in x_axis: from 0 to 1280
            if( (not(  (scale >= 2.0) and (((xbox_left >= 400) and (xbox_left <= 800)) or (xbox_left <= 150))  ))
                and
                (not(  (scale == 1.75) and (xbox_left >= 480) and (xbox_left <= 720)  ))
              ):
                searched_windows.append( ((xbox_left, ytop_draw+ystart),(xbox_left+win_draw, ytop_draw+win_draw+ystart)) )
    ...
    return heatmap, img_boxes, draw_img, searched_windows
